...
const Register = () => {
  ...
  const errorHandle = (error: AxiosError<any, any>) => {
    ...
    } else if (error?.response?.data?.message === 'Username is already in use') {
  ...
  const handleClick = async () => {
    try {
      setErr('');
      await myAxios.post(`/auth/register`, form.values);
      form.reset();
      navigate('/login');
    } catch (error) {
  ...
  const form = useForm({
    ...
    validate: {
      ...
      acceptRules: (value) => (value === false ? 'لطفا قوانین را قبول کنید' : null)
    }
  });
  return (
    <AuthLayout loginPage={false}>
      <Card miw="402px" shadow="0px 12px 50px rgba(0, 0, 0, 0.18)" radius="20px" p="24px">
        <Flex direction="column" align="center" gap="29px" bg="#fff">
          ...
          />
          <div style={{ width: '100%' }}>
            <Button h="48px" w="100%" fz="14px" fw="700" onClick={form.onSubmit(handleClick)}>
              ثبت‌نام
            </Button>
          </div>
        </Flex>
